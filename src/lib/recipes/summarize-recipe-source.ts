...
export const summarizeRecipeSource = async ({
	...
	const { output: recipeSummary } = await generateText({
		...
		system: [
			...
			'All human-readable output text must be concise natural Japanese.',
			'Keep only schema keys and null values in machine-readable form; all titles, summaries, ingredient lines, instruction lines, material names, preparation text, and amount.text must be Japanese.',
